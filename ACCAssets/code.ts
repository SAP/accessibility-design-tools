...
var selectedNodes: SceneNode[] = []; // existing Figma nodes in page
var createdNodes: SceneNode[] = []; // created Figma nodes in plugin

var focusGroup: GroupNode;
var readingGroup: GroupNode;
var landmarkGroup: GroupNode;
var headingGroup: GroupNode;

var gnode: GroupNode;  // To check if Group node was already generated

const messagemap = {  // Comm. object contains key-values for communication with HTML plugin
  zoomfactor: 1, // Default Zoom
...
if (figma.command === "focusorder") {
  ...
  var node: SceneNode = figma.currentPage.selection[0];
  if (node && node.type == "GROUP" && node.name == focusOrderGroupName) {
    ...
    for (let i = 0; i < gnode.children.length; i++) { 
      ...
      let attr: Array<string | string | string> = [name, id, variant];
...
if (figma.command === "readingorder") {
  ...
  if (node && node.type == "GROUP" && node.name == readingOrderGroupName) {
    ...
    for (let i = 0; i < gnode.children.length; i++) { 
      ...
      let attr: Array<string | string | string> = [name, id, multiple]; 
      messagemap.nodearray = attr;
      figma.ui.postMessage(messagemap);
    }
  }   
...
if (figma.command === "landmarks") {
  ...
  if (node && node.type == "GROUP" && node.name == landmarkOrderGroupName) {
    ...
    for (let i = 0; i < gnode.children.length; i++) { 
      ...
      let attr: Array<string | string | string> = [name, id, landmarkname]; 
      messagemap.nodearray = attr;
      figma.ui.postMessage(messagemap);
    }
  }   
...
if (figma.command === "headings") {
  ...
  if (node && node.type == "GROUP" && node.name == headingOrderGroupName) {
    ...
    for (let i = 0; i < gnode.children.length; i++) { 
      ...
      let attr: Array<string | string | string | string> = [name, id, level, invisible, invisibleText]; 
      messagemap.nodearray = attr;
      figma.ui.postMessage(messagemap);
    }
  }   
...
figma.ui.onmessage = async(msg) => {
  ...
  if (msg.type === 'addFocusOrder' || msg.type === 'addReadingOrder' ||  msg.type === 'addLandmarkOrder' ||  msg.type === 'addHeadingOrder') { 
    var node: SceneNode = figma.currentPage.selection[0];
    var attr: Array<string|string|string|string|string> = [];
    if (node) {
  ...
  if (msg.type === 'createFocusOrder') {    
    ...
    if (g != undefined) nodeSelect(g.id)

    // check if we have modded a Group, if so, delete the old group
    if (gnode != undefined) gnode.remove();   
  ...
  if (msg.type === 'createReadingOrder') {    
    for (let i = 0; i < msg.items.length; i++) {  
      ...
      if (node) {
        ...
        if (msg.bitems.findIndex((element: any) => element == msg.items[i]) != -1) 
  ...
  || msg.type === 'deleteLandmarkItem' || msg.type === 'deleteHeadingItem' ) {
    try {
      ...
      console.error('Delete Item: ', (ex as Error).message);
    }
  }  
  ...
  || msg.type === 'clearLandmarkOrder' || msg.type === 'clearHeadingOrder' ) {
    try {      
      ...
      console.error('Clear Order: ', (ex as Error).message);
    }
  }  
...
function createFocusStep (n: SceneNode[], i:number) {

  //var array = ["Single","Horizontal","Vertical","Both"]; // 1,2,3,4
  
  const t:Transform = n[i].absoluteTransform
  const x = t[0][2]
  const y = t[1][2]
  ...
  var fgroup: GroupNode;  

  //inner function
  function returnfGroup() {

    function getKeyRect(px: number, py: number) {
    ...
    function getKeySymbol(px: number, py: number, r: number) {
    ...
    if (n[i].getPluginData("FocusOrderVariant") == "1") { 
      let nodes: Array<EllipseNode | TextNode | RectangleNode> = [circle, label, rect]; // default
      fgroup = figma.group(nodes,figma.currentPage); 
    }

    if (n[i].getPluginData("FocusOrderVariant") == "2") { 
      let nodes: Array<EllipseNode | TextNode | RectangleNode | RectangleNode | RectangleNode | PolygonNode | PolygonNode> = 
        [circle, label, rect, getKeyRect(x + n[i].width/2 - 20 - 2, y + 2), getKeyRect(x + n[i].width/2 + 2, y + 2), 
    ...
    if (n[i].getPluginData("FocusOrderVariant") == "3") { 
      let nodes: Array<EllipseNode | TextNode | RectangleNode | RectangleNode | RectangleNode | PolygonNode | PolygonNode> = 
        [circle, label, rect, getKeyRect(x + n[i].width - 24, y + n[i].height/2 - 20 - 2), getKeyRect(x + n[i].width - 24, y + n[i].height/2 + 2), 
    ...
    if (n[i].getPluginData("FocusOrderVariant") == "4") { 
      let nodes: Array<EllipseNode | TextNode | RectangleNode | RectangleNode | RectangleNode| RectangleNode | RectangleNode | PolygonNode | PolygonNode | PolygonNode | PolygonNode> = 
        [circle, label, rect, getKeyRect(x + n[i].width/2 - 20 - 2, y + 2), getKeyRect(x + n[i].width/2 + 2, y + 2),
...
function createReadingStep (n: SceneNode[], i:number) {          
    ...
    var rgroup: GroupNode;
    let nodes: Array<RectangleNode|PolygonNode|TextNode> = [rect, hexagon, label]; 
...
function createLandmarkStep (n: SceneNode[], i:number) {          
  ...
  var lgroup: GroupNode;
  let nodes: Array<RectangleNode|RectangleNode|TextNode> = [rect, lrect, label]; 
...
function createHeadingStep (n: SceneNode[], i:number) {          
...
// Heading Role Arrow
  ...
  let nodes: Array<RectangleNode|FrameNode|PolygonNode|TextNode> = [rect, lframe, arrow, label]; 
  var hgroup: GroupNode;
...
function createStepText (scn: SceneNode, i:number, name:string) {          
...
function createFrameText (scn: SceneNode, i:number, name:string) {          
  const label = figma.createText();
  label.fontName = { family: "Inter", style: "Bold" }
  label.fontSize = stepFontSize;
  label.name = name + (i+1).toString();
